...
def addNetworks():
    ...
    while True:
        ...
        if newNetwork.lower() not in ("", "y", "yes"):
            break
...
def main():
    ...
    with open("topology.json", "w") as fil:
        fil.write(json.dumps(config, indent=2))
        fil.write("\n")
    print("topology saved to topology.json", file=sys.stderr)
