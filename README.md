...
2. **Fix board caching** - the current visual bug where the current user on the main board is incorrect sometimes needs to be fixed, as an admin user experiencing this bug would not be able to easily access admin-only pages like /manage_user_accounts without having to type in the path manually. Ideally, this fix would also include a slight change to how the board is generated so that the redis database isn't getting spammed constantly
3. **Stop PWNBoard from getting DDoSed** - with the amount of POST requests that PWNBoard receives every minute during full-blown engagements, the poor Flask app sometimes gets overwhelmed and will stop responding for a couple of seconds. Some sort of load balancing should be implemented to prevent this.
4. **Add option to disable access tokens** - some red teams might not want authorization to PWNBoard due to the extra step, so an option to disable this feature would be nice
5. **Persistent SQLite3 database** - currently a SQLite3 database (within the PWNBoard container) contains the user information. This file is generated by `__init__` when the Flask web app is started up. On destroy and rebuild, this database file will be deleted, so user accounts will have to be remade. I can't think off the top of my head the best way to accomplish persistent storage of user information, but this would be a great feature.
